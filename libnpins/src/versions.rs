...
pub fn upgrade(mut pins_raw: Map<String, Value>, path: &Path) -> Result<Value> {
    ...
    let version_upgraders: BTreeMap<u64, Upgrader> = [
        ...
        (
            7,
            Box::new(|pins_raw: &mut Map<String, Value>| {
                /* This is a modified copy of `generic_upgrader` to provide a warning */
                let pins = pins_raw
                    .get_mut("pins")
                    .and_then(Value::as_object_mut)
                    .with_context(|| {
                ...
                log::warn!("Tarball pins dropped support for the \"Lockable HTTP Tarball Protocol\". Instead, locking and updating is now implemented through HTTP redirects. In most relevant setups, which should be a drop-in replacement with no further action needed. However, you are advised to inspect all pins of type `Url` and `Mutable` url (formerly `Tarball`) after the upgrade.");
                let tarball_pins = pins.iter()
                    .filter(|(_, value)| {
                        value
                            .as_object()
                            .and_then(|o| o.get("type"))
                            .and_then(Value::as_str)
                            .is_some_and(|ty| ty == "Tarball")
                    })
                    .map(|(name, _)| name.as_str())
                    .collect::<Vec<_>>();
                log::warn!("Tarball pins that may need manual inspection in this lock file: {tarball_pins:?}");
                for (name, pin) in pins.iter_mut() {
                    upgrade_v7_pin(
                        name,
                        pin.as_object_mut()
