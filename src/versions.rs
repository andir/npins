...
pub fn upgrade(mut pins_raw: Map<String, Value>) -> Result<Value> {
    ...
    } else if version_upgraders.range(version..).next().is_none() {
        /* We are not at the latest version, but there is no upgrader registered, which means that
         * the upgrade is already being handled by serde default fields. The diff will always be
         * the bump of the version field and nothing more.
         */
        log::info!("There is nothing to do");
    } else {
        /* This is where the upgrading happens. */
